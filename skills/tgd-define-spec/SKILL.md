...
2. **A named tracking event that does not exist yet** — write the event name (e.g. `sign_up_completed`). `/tgd-plan` will register it in `$TGD_DIR/TRACKING-PLAN.md` and create an instrumentation task with its own acceptance criteria (see `tgd-plan-breakdown`).
...
For modes 2-3, run `tgd-define-sketch`, save the required variants under `$TGD_DIR/<feature-name>/prototype/`, visually verify each one, present the comparison, and let the Design owner select or request a hybrid. Mode 1 skips `tgd-define-sketch` entirely.
...
- **Plan + Tasks** → `/tgd-plan` running `tgd-plan-breakdown`.
...
- **Implement** → `/tgd-develop` running `tgd-develop-incremental` or
  `tgd-develop-subagents`, with `tgd-develop-tdd` and
  `tgd-core-context`.
