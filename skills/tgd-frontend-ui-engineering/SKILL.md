---
name: tgd-frontend-ui-engineering
...
**Before implementing any UI, read `$TGD_DIR/CONTEXT.md`, the real UI source files it points to, and `$TGD_DIR/<feature-name>/DESIGN.md`.** Apply this exact precedence: **Existing design system > approved DESIGN.md additions > tGD fallback defaults**. CONTEXT.md locates the sources; it does not replace them. DESIGN.md owns the approved feature-specific direction, user flow, component mapping, states, and justified additions. This skill covers code architecture, component patterns, state management, and accessibility — it must not silently replace the product's visual system.
