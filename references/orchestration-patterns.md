...
```
/tgd-review → code-reviewer (with tgd-code-review-and-quality skill) → report
```

**Use when:** the same single-persona invocation happens repeatedly with the same setup.

**Examples in this repo:** direct persona invocation, or `/tgd-review` on low-stakes features (single five-axis pass).
...
```
                    ┌─→ code-reviewer    ─┐
/tgd-review → fan out  ───┼─→ security-auditor ─┤→ merge → go/no-go + rollback
                    └─→ test-engineer    ─┘
```
...
**Examples in this repo:** `/tgd-review`'s Persona Fan-Out step.
...
```
user runs:  /tgd-define → /tgd-plan → /tgd-develop → /tgd-verify → /tgd-review → /tgd-release
```
...
In Claude Code, parallel fan-out (Pattern 3) requires issuing **multiple Agent tool calls in a single assistant turn**. Sequential turns serialize execution. `/tgd-review`'s Persona Fan-Out calls this out explicitly. Any new orchestrator command should do the same.

---

## Worked example: Agent Teams for competing-hypothesis debugging

This example shows when to reach for **Agent Teams** instead of `/tgd-review`'s subagent fan-out. The two patterns look similar from a distance — both spawn the same three personas — but the value comes from a different place.
...
A single agent will pick the first plausible theory and stop investigating. A `/tgd-review`-style subagent fan-out would have each persona report independently — but their reports never meet, so nothing rules out the wrong theories.
...
### Why this is *not* a review-fan-out job

| | `/tgd-review` fan-out (subagents) | Agent Teams |
|--|--------------------|-------------|
...
The review fan-out is a verdict; Agent Teams is an investigation.
...
Three Sonnet teammates running for ~10–15 minutes of investigation costs noticeably more than the same three personas spawned as subagents by `/tgd-review`. The justification is *quality of conclusion* — for production debugging where the wrong fix is expensive, the extra tokens are a bargain. For a routine PR review, stick with the `/tgd-review` fan-out.
...
- Production-bound verdict on a known diff → use `/tgd-review`'s fan-out (subagents).
...
A hypothetical command calls a `pre-ship-coordinator` that calls a `quality-coordinator` that calls `code-reviewer`.
