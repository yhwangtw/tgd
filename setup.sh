...
# tGD Wiki — powered by the tgd-wiki-generation skill
# No install needed here: /tgd-map Step 6 generates a self-contained
# wiki.html with python3 stdlib only (Mermaid renderer is vendored in the
# skill). No node/npm, no build step.
echo "📖 tGD Wiki: single-file wiki.html is generated by /tgd-map Step 6 (python3 only)."
