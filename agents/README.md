...
- `/tgd-review` → runs the five-axis review (and fans out to all three personas for high-stakes features)
- `/tgd-verify` → runs the test/regression/traceability gates (invoke `test-engineer` directly for coverage strategy)
...
- `/tgd-review`'s Persona Fan-Out → runs `code-reviewer` + `security-auditor` + `test-engineer` in parallel, then synthesizes their reports into REVIEW.md
...
└── No  → Are the sub-tasks independent (no shared mutable state, no ordering)?
         ├── Yes → Slash command with parallel fan-out (e.g. /tgd-review's Persona Fan-Out)
         └── No  → Sequential slash commands run by the user (/tgd-define → /tgd-plan → /tgd-develop → /tgd-verify → /tgd-review)
```

## Worked example: valid orchestration

`/tgd-review`'s Persona Fan-Out step is the canonical fan-out orchestrator in this repo:

```
/tgd-review (Persona Fan-Out)
...
- The user already knows they want a review; let them call `/tgd-review` (or the persona) directly
...
- **As subagents:** auto-discovered when this plugin is enabled (no path config needed). Use the Agent tool with `subagent_type: code-reviewer` (or `security-auditor`, `test-engineer`). `/tgd-review`'s Persona Fan-Out step is the canonical example.
