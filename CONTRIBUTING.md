...
- **Zip file**: Must match directory name exactly: `{skill-name}.zip`. **Only needed for distribution** — during development, work directly with the skill directory. Zip files are generated manually with `cd skills && zip -r {skill-name}.zip {skill-name}/`.
